...
#Task 14: Guest OS can view HTTP and HTTPS pages, but apache2 server is blocked from serving HTTP
content.
...
$IPT -A INPUT -p tcp --dport 443 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT#Task 23: SSH from
Client A to Server A
$IPT -A INPUT -p tcp -s 192.168.60.111 --dport 22 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT#Task 26: Change iptables to forward packets
